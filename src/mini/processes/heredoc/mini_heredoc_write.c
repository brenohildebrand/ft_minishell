...
/*   Updated: 2024/06/12 21:37:06 by bhildebr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_none	mini_heredoc_write(t_mini mini)
{
	...
	mini_assert(mini, fd != -1, HEREDOC_FILE_ERROR);
	...
	if (bytes_written == -1 || bytes_written != cstring_get_length(mini->heredoc->buffer))
		close(fd);
	mini_assert(mini, bytes_written != -1, HEREDOC_FILE_ERROR);
	mini_assert(mini, bytes_written == cstring_get_length(mini->heredoc->buffer), HEREDOC_FILE_ERROR);
	close(fd);
