...
/*   Updated: 2024/06/30 22:59:24 by bhildebr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_cstring	get_filepath_from_redir(t_mini mini, t_mini_list node)
{
	...
	if (raw_filepath[0] == '/')
	{
		return (cstring_copy(mini, raw_filepath));
	}
	else if (raw_filepath[0] == '.' && raw_filepath[1] == '/')
	{
		tmp = cstring_remove(mini, raw_filepath, 0, 0);
		cwd = mini_get_cwd(mini);
		filepath = cstring_join(mini, tmp, cwd);
		mini_free(mini, tmp);
		mini_free(mini, cwd);
	}
	else
	{
		cwd = mini_get_cwd(mini);
		tmp = cstring_join(mini, cwd, "/");
		filepath = cstring_join(mini, tmp, raw_filepath);
		mini_free(mini, tmp);
		mini_free(mini, cwd);
	}
	return (filepath);
